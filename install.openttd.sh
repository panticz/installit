...
if [ "$1" == "-n" ]; then
  ...
  if [ "$(uname -m)" == "x86_64" ]; then
    # 64 bit
    URL=http://binaries.openttd.org/releases/1.5.0/openttd-1.5.0-linux-ubuntu-trusty-amd64.deb
  else
    # 32 bit
    URL=http://binaries.openttd.org/releases/1.5.0/openttd-1.5.0-linux-ubuntu-trusty-i386.deb
  fi
